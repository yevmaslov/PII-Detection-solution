...
def filter_student_preds(row):
    ...
    if not "NAME_STUDENT" in row["label"]:
        ...
        try:
            if (row["token_str"].istitle() or row["token_str"] == "\n" or row["token_str"] == "-") and (not any(x.isdigit() for x in row["token_str"])) and (not any(x == "_" for x in row["token_str"])):
                return True
            else:
                return False
        except:
        ...
        if 1 + 1 == 2: #"ID_NUM" in sub.label[i] or "PHONE_NUM" in sub.label[i]:
            ...
            try:
                if re.match(digit_pat, string_to_check):
                    ...
                    if (len(string_to_check) == 9 or len(string_to_check) >= 11) and "PHONE_NUM" in sub.label[i]:
...
def postprocess_username(df):
    ...
    sub = df
    for i in range(len(sub)):
        if i+2 < len(sub) and sub.label[i] == "B-USERNAME" and sub.label[i+1] == "B-USERNAME" and sub.label[i+2] == "B-USERNAME" and \
        sub.document[i] == sub.document[i+1] and sub.document[i] == sub.document[i+2] and sub.token[i+1] == sub.token[i] + 1 and sub.token[i+2] == sub.token[i] + 2 and \
        sub.token_str[i+1] in ["-", "."]:
...
def label_postprocessing(df, doc2tokens, data):
    ...
    df = df.drop_duplicates(subset=["document", "token"], keep="first")
    df = df[~((df["label"].str.contains("EMAIL")) & (~df["token_str"].str.contains("@")))] #ok 
